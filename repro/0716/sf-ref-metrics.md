...
|---|---:|---:|---:|
| QVG（INT2，195 匹配配置） | 38.65 | 0.9736 | 0.0413 |
| **PCA-KV N4** | 38.52 | 0.9730 | 0.0427 |

> **SSIM 勘误（0716 深夜）**：本表初版 SSIM 0.9991/0.9990 出自 `sf_ref_metrics.py`
> 的整帧单窗"全局 SSIM"（严重虚高，见 [hy-ref-metrics.md](hy-ref-metrics.md) §勘误）；
> 已换 paper 原版 11×11 局部窗实现重算（结论不变：仍打平）。脚本已修复。
...
已完成，见 [hy-ref-metrics.md](hy-ref-metrics.md)——注意该文档经历了一次协议级修正
（[23,36) 跨崖窗口 → 平台期协议），HY 结论为 **N4 输 3.1 dB**，与本文档的 SF 打平、
LC 大胜并列为三模型画像。
