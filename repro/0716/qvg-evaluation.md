...
| **LPIPS** | 有参考 | 同上，VGG 感知距离 | **官方实现把 [0,1] 帧直喂 lpips（未归一到 [−1,1]，`metric.py:131`）**——复现必须复刻这个怪癖，否则数值偏高 ~2×（详见 [ssim-lpips-validation.md](ssim-lpips-validation.md)） |
...
4. **LPIPS 输入范围怪癖**：官方 metric.py 未做 [−1,1] 归一化——首帧 SSIM 可对上 paper、
   LPIPS 需按官方口径复刻后才接近（残余 ~0.02-0.03 未归零，排序一致）
5. **工具教训**：`pdftotext` 对本 PDF 附录页抽取为空——"全文没有 X"类断言必须用页面
   渲染复核（本文档初版因此漏掉附录 A）
