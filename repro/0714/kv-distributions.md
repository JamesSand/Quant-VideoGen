...
- 结构注记（0714 更正）：HY 原生是**双分支注意力**——rope 分支（标准旋转编码）+
  **PRoPE 分支**（Projective Positional Encoding，相机投影几何编码，Q/K/V 三者都被变换，
  零初始化 adapter 输出头；HY 相机控制的核心机制，非 QVG 所加）。QVG 的集成把两分支的
  K（和 V）**沿 channel 拼成 256 维**塞进一个 ChunkedKVCache
  （`arwan_w_action_w_mem_relative_rope.py:179-184`，读取时 chunk(2) 拆回），量化作用于
  拼接后的整体。此前"[RoPE 后‖pre-RoPE]"的说法有误——第二半区是 PRoPE 分支，不是未编码的 K。
