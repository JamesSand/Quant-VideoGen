...
读数：9 格 = Q/K/V（行）× 开头/中段/结尾（列），每格横轴 token index、纵轴 token norm
（细线 = 12 个 head 各自、粗线 = 跨 head 中位）。三列曲线形态与水平几乎完全一致——
Q 中位 14.7→15.3→15.4、K 14.4→14.7→14.9、V ≈11.8，无漂移、无新增离群 head。
...
读数：9 格 = Q/K/V（行）× L0/L15/L29（列），逐 token 曲线。**K—L29 格里可以直接看到
那个离群 head：一条悬在 norm≈105 的独立水平带（H9），全 token 均匀、与其他 11 个 head
（~15 附近）完全分离**；Q—L29 也有若干 head 形成 40-75 的弥散高带。L0/L15 两列干净，
V 行三列无差。同层同窗下 H4 与 H9 的 3D 曲面对比（下图）：
