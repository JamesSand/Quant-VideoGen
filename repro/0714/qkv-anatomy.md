...
读数：9 格 = Q/K/V（行）× 开头/中段/结尾（列），OScaR Fig.3 口径——每个 token 位置一个
箱子（该 token 跨 12 head 的 norm 分布）。三列箱形几乎完全一致，无漂移、无位置性离群。
...
读数：9 格 = Q/K/V（行）× L0/L15/L29（列），OScaR Fig.3 口径。**K—L29 格里每个位置的
箱子上方都悬着一个 ≈105 的离群点——就是 H9，任何 token 位置都在，证明是整头离群而非
个别 token**；Q—L29 同样每个位置有 50-70 的离群点。L0/L15 两列干净，V 行三列无差。
同层同窗下 H4 与 H9 的 3D 曲面对比（下图）：
