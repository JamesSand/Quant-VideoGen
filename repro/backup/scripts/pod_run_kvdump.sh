...
export TRITON_CACHE_DIR=$PWD/repro/backup/triton_cache/kvdump
mkdir -p $TRITON_CACHE_DIR repro/backup/race repro/backup/logs results/ropestudy
RESULT=repro/backup/race/result_kvdump.txt
...
torchrun --nproc_per_node=1 --standalone experiments/Self-Forcing/inference.py \
  ...
  --data_path repro/backup/scripts/prompt0.txt --output_folder results/ropestudy/video \
  --num_samples 1 --num_output_frames 180 --local_attn_size 180 \
  --use_ema --save_with_index --quant_type none \
  > repro/backup/logs/kvdump.log 2>&1
RC=$?
