...
mkdir -p $TRITON_CACHE_DIR repro/backup/race

touch repro/backup/race/release_gpu${GPU}   # tell the sentinel to start decaying
...
if [ "$KIND" = "lc" ]; then
  if [ "$ROT" = "0" ]; then
    ...
    CUDA_VISIBLE_DEVICES=$GPU bash repro/backup/scripts/run_quarot_longcat.sh $BITS $BLOCK $SYM $ROT $TAG \
      > repro/backup/logs/quarot_$TAG.log 2>&1
  fi
  RC=$?
  ...
  if [ "$ROT" = "0" ]; then
    CUDA_VISIBLE_DEVICES=$GPU QUAROT_DISABLE=1 bash repro/backup/scripts/run_quarot_hy.sh $BITS $BLOCK $SYM 0 $TAG \
      > repro/backup/logs/quarot_$TAG.log 2>&1
  else
    CUDA_VISIBLE_DEVICES=$GPU bash repro/backup/scripts/run_quarot_hy.sh $BITS $BLOCK $SYM $ROT $TAG \
      > repro/backup/logs/quarot_$TAG.log 2>&1
  fi
  RC=$?
  ...
  if [ -s results/quarot/$TAG/err.txt ]; then
    echo "RESULT $TAG FAILED err.txt: $(head -1 results/quarot/$TAG/err.txt)" >> repro/backup/race/results.txt
    exit 1
  fi
fi

if [ $RC -ne 0 ] || [ ! -f "$OUT" ]; then
  echo "RESULT $TAG FAILED rc=$RC out_missing=$([ -f "$OUT" ] && echo no || echo yes)" >> repro/backup/race/results.txt
  exit 1
fi
...
echo "RESULT $TAG OK psnr=$PSNR ssim=$SSIM lpips=$LPIPS" >> repro/backup/race/results.txt
