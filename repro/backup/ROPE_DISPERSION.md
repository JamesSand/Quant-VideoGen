...
方法要点：post-RoPE 用模型自身的 `rope_params/causal_rope_apply` 重构（与推理路径逐位一致），旋转保范数验证到 3.7e-16；pre/post 用同一子采样、同一 k-means 种子；分析脚本 `repro/ropestudy/step{1,2}_*.py`，数据 `repro/ropestudy_data.npz`。
