# Report 0721 — Budget-PCA:方法 · 战绩 · Kernel · Why(基线 = 0720 终版,含 0721 归因修正)

> 本文件为 [../0720/report-0720.md](../0720/report-0720.md) 的同步副本(链接指回 0720 资产);0721 新增:§4.1 归因修正(grid_cross)+ pcatern/qvg4pt/kivipost 反事实臂 + 实存记账规则。

> 可直接 present 的版本。当日叙事与勘误过程见 [README.md](../0720/README.md) /
> [HANDOFF-0720.md](../0720/HANDOFF-0720.md);复现指南见 [REPRODUCE-0720.md](../0720/REPRODUCE-0720.md)。
...
代入三模型,与 kernel **逐字节实测**([bpe-audit.md](../0720/bpe-audit.md))对账:
...
VBench)。每列量化方法间最优加粗。(源:[mp100-table.md](../0720/mp100-table.md))
...
> 详见 [paper-diff-plan.md](../0720/paper-diff-plan.md)。
> ᵇ QVG 的 BPE 为其实现的**逐字节实测**(质心实存 fp32):LC 2.464 /
> SF 2.406 / HY 3.320——paper 名义 2.326 与其实现对不上(未计质心真实摊销)。
> 记账规则:baseline 一律按其代码实存计,不做任何代为优化;我们的 BPE
> 同为逐字节实测(含归一因子,§1.3/§五)。
...
- **encode**([kernel/bp_quant.py](../0720/kernel/bp_quant.py)):单个 `torch.compile`
  整图——mean → bf16 协方差 GEMM → **5 轮 Cholesky 正交化子空间迭代**(取代
  批量 eigh 的 82ms,能量 99%+)→ 系数 2-bit+fp8 → 残差按轴分块 2-bit+fp8 →
  位打包;每(配置,形状)编译一次缓存复用;
- **decode**([kernel/bp_triton.py](../0720/kernel/bp_triton.py)):手写 Triton 融合
...
后续工作。(全文:[kernel-results.md](../0720/kernel-results.md))
...
[why-budget-pca-wins.md](../0720/why-budget-pca-wins.md),证伪与勘误
[why-refuted-and-errata.md](../0720/why-refuted-and-errata.md)):
...
![fig3:残差格效率——QVG 回收 ~48%(其三电平格的输入无关天花板)vs 我们 ~77%](../0720/why/fig3_residual_efficiency.png)
...
跌破 QVG 27.61;②QVG 换四电平 asym B64(qvg4pt 臂)→ 27.61 涨到 **32.85(⚠0721 勘误中:此数系 qvg4pt 臂漏设 fp8 元数据模拟的口径,scale/zp 实为 fp32、真实 BPE ~2.84;fp8 合法口径复测进行中,完成后更新)**
反超我们 1.2dB,但 BPE **2.589**(实存口径,超预算 11%;zp 翻倍残差元数据)
且 encode 慢 33×——它的字典太贵,预算内养不起好格子;我们的减法便宜,省下的钱恰好
养满四电平格。**chunk 级最终误差
...
![fig1:K 协方差谱——低秩薄饼层依赖](../0720/why/fig1_spectra.png)
...
![fig4:逐通道误差——QVG 的损伤集中在小方差通道](../0720/why/fig4_channel_error.png)

**H3 维度诅咒(成立)**:连续 token 云撒 256 个质心盖不满;码本 ×16 只多消
+0.3~+8.6pp,收益指数衰减(率失真:VQ 需 $`K\approx 2^{nR}`$ 追平变换编码):

![fig5:同一片 token 云——左 kmeans 撒点覆盖(位移线=图内可见误差),右 PCA 连续子空间(主方向零误差)](../0720/why/fig5_pc_plane.png)
...
![fig6:HY K 半区分谱——prope 更低秩、能量更大,但秩 9:0 给 rope 反而赢](../0720/why/fig6_hy_halves.png)
...
| ② | QVG 的 HY 真实 BPE ≈2.738,非表中 2.326 | **属实且更甚**:逐字节实测其实现**质心存 fp32**——LC 2.464 / SF 2.406 / **HY 3.320**(其 paper 名义 2.326 与实现不符) | 终表 QVG 列改为实测值 + 脚注;从此双方 BPE 一律同口径逐字节实测 |
