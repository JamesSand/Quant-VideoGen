#!/usr/bin/env python3
"""聚合 SF qvg-nom 判决:qvgprot/qvgproc vs mp100 既有臂(ours=pcaa128kaxvaxfp8 终版)。
...
cache = {}
for f in sorted(glob.glob("repro/0718/npz/vbench4_*shard*.json")) + \
         ["repro/0718/npz/vbench4.json"]:
    if os.path.exists(f):
        cache.update(json.load(open(f)))

ARMS = ["bf16", "kivi", "qvg", "pcaa128kaxvaxfp8", "qvgprot", "qvgproc"]
AX = ["BC", "IQ", "SC", "AQ"]
...
L += ["", "## 配对符号检验(vs ours=pcaa128kaxvaxfp8(终版臂),同 prompt 双向)", "",
      "| 对比 | 轴 | win/tie/loss(对方视角) | p(双侧) |", "|---|---|---|---|"]
for a in ["qvgprot", "qvgproc"]:
    common = sorted(set(per[a]) & set(per["pcaa128kaxvaxfp8"]))
    for x in AX:
        w = sum(1 for p in common if per[a][p][x] > per["pcaa128kaxvaxfp8"][p][x])
        l = sum(1 for p in common if per[a][p][x] < per["pcaa128kaxvaxfp8"][p][x])
        t = len(common) - w - l
