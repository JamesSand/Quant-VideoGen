...
NPZ = "repro/0721/npz"
NPZ_HY = "repro/0718/npz"   # HY 沿用 mp100

vb = {}
for f in sorted(glob.glob(f"{NPZ}/vbench4_mpfull_shard*.json")) + sorted(glob.glob(f"{NPZ_HY}/vbench4_shard*.json")) + [f"{NPZ_HY}/vbench4.json"]:
...
BPE = {"rtn": "2.25", "kivi": "2.25", "rtnfp8": "2.25", "kivifp8": "2.32", "quarot": "2.25", "qvg": "2.464ᵇ",  # ours = audited bytes (kernel-results)
       "pcakaxvaxfp8": "**2.319**", "pcaa128kaxvaxfp8": "**2.318**", "pcav90kpternkaxkb64fp8": "**2.325**", "bf16": "16"}
LABEL = {"bf16": "BF16 KV(参考)", "rtn": "RTN", "kivi": "KIVI", "rtnfp8": "RTN", "kivifp8": "KIVI", "quarot": "QuaRot",
         "qvg": "QVG", "pcakaxvaxfp8": "**Budget-PCA(r4+双通道轴)**",
         "pcaa128kaxvaxfp8": "**Budget-PCA(r4+双通道轴)**", "pcav90kpternkaxkb64fp8": "**Budget-PCA(K9:0/V9:0+K通道轴B64+KP三值)**"}

def collect(model):
    rows, ns = {}, {}
    if model == "lc":
        arms = ["bf16", "rtnfp8", "kivifp8", "quarot", "qvg", "pcakaxvaxfp8"]
        for arm in arms:
            ref_vals, vbs = [], []
            for p in range(1, 1004):
                ...
                if arm != "bf16":
                    f = f"{NPZ}/mpfull_lc_p{p}_{arm}.npz"
                    if os.path.exists(f):
                        z = np.load(f)
                        if len(z["psnr"]) > 93:
        ...
        arms = ["bf16", "rtnfp8", "kivifp8", "quarot", "qvg", "pcaa128kaxvaxfp8"]
        for arm in arms:
            vbs = []
            for p in range(1, 1004):
        ...
        arms = ["bf16", "rtn", "kivi", "quarot", "qvg", "pcav90kpternkaxkb64fp8"]
        for arm in arms:
            ref_vals, vbs = [], []
            for s in range(10):
...
for model, title in (("lc", "LongCat-Video-13B(1003 全量 prompts × f93)"),
                     ("hy", "HY-WorldPlay-8B(10 seeds × frames[13:];沿用 mp100,不随 prompt 集变化)"),
                     ("sf", "Self-Forcing-Wan(1003 全量 prompts × 700 帧窗)")):
    rows, ns = collect(model)
    sec, verd = fmt_table(model, title, rows, ns)
    sections.append(sec); all_verdicts += verd

hdr = ("# MPFULL 定案表(QVG Table-1 指标集,MovieGen 全量 1003 prompts)\n\n"
...
open("repro/0721/mpfull-table.md", "w").write(hdr + "\n\n".join(sections) + verdict_txt + "\n")
print(hdr + "\n\n".join(sections) + verdict_txt)
