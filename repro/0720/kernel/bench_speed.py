...
for model in ("lc", "sf", "hy"):
    ...
    for ci in range(NCH):
        ...
        if model == "hy":
            ...
            def enc_k(): return {"halves": (bp_encode_fast(k1, r=9, grid="asym", block=64, axis="channel", cmode="reduce-overhead"),
                                            bp_encode_fast(k2, r=0, grid="ternary", block=64, axis="channel", cmode="reduce-overhead"))}
            def enc_v(): return {"halves": (bp_encode_fast(v1, r=9, grid="asym", block=128, axis="token", cmode="reduce-overhead"),
                                            bp_encode_fast(v2, r=0, grid="asym", block=128, axis="token", cmode="reduce-overhead"))}
