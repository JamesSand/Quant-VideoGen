...
3. **why 机制判决**([why-budget-pca-wins.md](why-budget-pca-wins.md),只留
   成立假说,每节带"大白话+公式+例子"导读框):真机制 = **残差格效率分工**
   (0721 归因修正)——QVG 发布实现的 int2 是三电平对称格,46-49% 回收率是
   该格子的输入无关天花板(在我们残差上同样 48%);同预算归因分解:电平数+
   非对称 +20pp、通道轴 +6-10pp、残差结构 +4pp(`why/grid_cross.py`);
   格效率差 ~2.2×,chunk 级最终误差我们 20/24 格更小;H2 通道劫持成立
   (LC 1.8-2.4×,8/8 跨层稳)、H3 维度诅咒成立、H4 排除"没调好"(iters
   ×50 换 ±0.5dB);证伪/撤回/勘误全部在
