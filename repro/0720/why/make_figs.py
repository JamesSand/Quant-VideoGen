...
for off, key, col, lab in ((-w/2, "realpath", "#B5484A", "QVG(三电平对称格,as-released)"),
                           (w/2, "ourspath", "#4A7AB5", "Budget-PCA(四电平非对称+通道轴)")):
    means, lo, hi = [], [], []
    for m in MODELS:
        rows = d[f"{m}_{key}"]
        if key == "realpath":
...
ax.set_title("真机制(0721 归因修正):QVG 回收 ~48%(其三电平格的输入无关天花板)vs 我们 ~77%")
