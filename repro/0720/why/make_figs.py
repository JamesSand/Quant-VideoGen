...
for m in MODELS:
    ...
    ax.semilogy(np.arange(1, len(s) + 1), s / s[0], color=C[m],
                label=f"{MNAME[m]} top-4 能量:chunk_001 {tops[0]*100:.0f}%(跨层 {min(tops)*100:.0f}–{max(tops)*100:.0f}%)")
...
for ax, m in zip(axes, MODELS):
    ...
    for i, (ci, mk) in enumerate(zip((1, 0, 3, 6), "osv^")):
        sel = rp[(rp[:, 0] == ci) & (rp[:, 1] == 256)]
        km_bits = 8 / D + 256 * 16 / S
        ax.plot(km_bits, sel[0, 2] * 100, mk, color="#B5484A",
                label="k-means K=256(per-head 全 D 维)" if i == 0 else None)
        osel = op[op[:, 0] == ci]
        ax.plot(ours_bits, osel[0, 1] * 100, mk, color="#4A7AB5",
                label=f"μ+PCA r={r_eff}(终版)" if i == 0 else None)
    ax.set_title(f"{MNAME[m]}(marker=chunk 001/000/003/006)")
...
fig.suptitle("H1 判决(真实口径):LC/SF 同等比特下字典减得仍更多(证伪不变);HY 字典元数据 4×", y=1.03)
...
ax.set_title("真机制(修正口径):QVG 回收 ~48%(白噪声水平)vs 我们 ~75%,~2× 且 12 格全稳")
...
for m in MODELS:
    ...
    mc_note[m] = f"跨层比值 {r[:,3].min():.1f}–{r[:,3].max():.1f}×"
fig, axes = plt.subplots(1, 3, figsize=(13, 3.8), dpi=150)
for ax, m in zip(axes, MODELS):
...
fig.suptitle("H2 判决:QVG 误差集中在小方差通道;LC 判决性(~2.2×)且跨层稳", y=1.03)
