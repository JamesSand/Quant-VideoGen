...
→ 27.61 涨到 **32.85**,反超我们 1.2dB——**但代价是 BPE 2.451(质心 bf16
宽松口径)/2.589(其 fp32 实存口径),比我们多花 0.13-0.27 bits 且超 2.326
预算**(asym 的 zp 让残差元数据翻倍到 0.25,预算内装不下),encode 还慢
33×。两个方向共同证明:η 由格子决定、减法强者在同格子下占优——QVG 的
