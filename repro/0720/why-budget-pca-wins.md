...
我们(77%),且其字典元数据实测已超预算(2.46-3.32)。**端到端双向验证**(10 prompts,f93):
①把我们的残差格换成同款三电平(pcatern 臂)→ 31.7 掉到 **23.97**,跌破
QVG(27.61);②把 QVG 的残差格换成四电平 asym B64(qvg4pt 臂,减法不动)
→ 27.61 涨到 **32.85(⚠0721 勘误中:此数系 qvg4pt 臂漏设 fp8 元数据模拟的口径,scale/zp 实为 fp32、真实 BPE ~2.84;fp8 合法口径复测进行中,完成后更新)**,反超我们 1.2dB——**但代价是 BPE 2.589(实存口径:
质心 fp32 + asym B64 的 zp 让残差元数据翻倍到 0.25),比我们多花 0.27 bits、
超 2.326 预算 11%**,encode 还慢 33×。两个方向共同证明:η 由格子决定、减法强者在同格子下占优——QVG 的
真正问题是它的字典减法太贵(质心元数据+慢),预算内养不起一个好格子;
我们的减法便宜(0.19 bits、无迭代),省下的预算恰好养满四电平格。初版"kmeans 残差
