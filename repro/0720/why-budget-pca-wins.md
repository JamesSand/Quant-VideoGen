...
| 模型 | $`\eta^{\mathrm{QVG}}`$(kmeans 残差→int2 B64) | $`\eta^{\mathrm{ours}}`$(结构化残差→通道轴格) | $`\eta^{\mathrm{QVG}}/\eta^{\mathrm{ours}}`$ |
|---|---|---|---|
| LC | 0.52 [0.51-0.54]($`1{-}\eta`$ ≈ 48%) | **0.24 [0.21-0.33]**($`1{-}\eta`$ ≈ 76%) | **2.2×** |
...
6.2 vs 10.4%)。4 个例外格如实记录:SF chunk_000(0.177 vs 0.158%,两家近
