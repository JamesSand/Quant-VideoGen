...
- **一次只跑一个 gpu_queue 实例**:显存预检是"检查后启动",两个并发队列会同时
  通过检查 → 双订 OOM(0719 一次损失 60 任务返工)。多队列需求一律合并成单一
  jobs 文件;
- HY 的 chnorm(通道 σ 归一)已证伪(16.95 vs 基线 17.54)——post-transform 数据
  的通道方差是位置混合的,归一化有害;channel 轴(kax)才是 HY 的正解。
