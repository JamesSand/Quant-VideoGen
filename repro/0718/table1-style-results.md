...
- **Baseline 行**(RTN/KIVI/QuaRot):RTN=逐 64 块非对称 2-bit(fork naive-int2 同款
  数学,经我们带 SF store-fix 的 launcher);KIVI=K 逐通道 g128 / V 逐 token g64
  非对称 2-bit(其 FP 残差窗对应管线原生 BF16 近期窗);QuaRot=Hadamard 旋转+均匀
  2-bit(repro/backup/scripts/quarot_launcher.py,0716 版)。数字在补(每模型×每
  baseline 冒烟已跑,全量 40 run/baseline 待用户确认后跑);旧协议参考点:QuaRot
  @LC 单 prompt 30.38(0716,几何不同勿混填);
