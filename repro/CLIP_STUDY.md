...
> 结论：**纯丢弃式裁剪在所有阈值下都严格有害**，不存在最优裁剪点；离群值本身就是注意力的关键信号。
...
"加 clipping 让 INT2 KV 更好"——**否**（纯丢弃式）。absmax 不裁剪虽然"浪费"分辨率在离群值上，但保住它们比精细分辨普通值重要一个数量级。若要利用离群值结构，正确方向是**提取+稀疏旁路**，不是裁剪。
