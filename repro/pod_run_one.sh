...
. .venv/bin/activate
. repro/env_fix.sh
export TRITON_CACHE_DIR=$PWD/repro/triton_cache/pod_$TAG
mkdir -p $TRITON_CACHE_DIR repro/race repro/logs repro/metrics
...
if [ "$KIND" = "lc" ]; then
  if [ "$ROT" = "0" ]; then
    ...
    bash repro/run_quarot_longcat.sh $BITS $BLOCK $SYM $ROT $TAG > repro/logs/quarot_$TAG.log 2>&1
  fi
  RC=$?
  ...
  if [ "$ROT" = "0" ]; then
    QUAROT_DISABLE=1 bash repro/run_quarot_hy.sh $BITS $BLOCK $SYM 0 $TAG > repro/logs/quarot_$TAG.log 2>&1
  else
    bash repro/run_quarot_hy.sh $BITS $BLOCK $SYM $ROT $TAG > repro/logs/quarot_$TAG.log 2>&1
  fi
  RC=$?
...
if [ $RC -ne 0 ] || [ ! -f "$OUT" ]; then
  echo "FAILED rc=$RC out_missing=$([ -f "$OUT" ] && echo no || echo yes) last_err=$(grep -m1 -oE 'OutOfMemoryError|ModuleNotFoundError[^\"]*|RuntimeError[^\"]*' repro/logs/quarot_$TAG.log | head -1)" >> $RESULT
  exit 1
fi
