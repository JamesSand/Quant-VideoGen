...
0. **/goal 达成：N12 方法**（[n12-results.md](n12-results.md)·[research-log.md](research-log.md)·
   [eval-protocol.md](eval-protocol.md)）——K/V 双侧在线字典 + asym B128 残差，
   LC/HY/SF 三关全超 QVG 且 BPE 更低；途中证伪 N5-N9 五个候选、完成两次张量级诊断
   （HY 的 K 与 V 都吃字典）、修复 fp8 溢出 NaN。

1. **HY 断崖可视化检验**（视频 `videos/`、图 `figs/`）：确认 HY **无 LongCat 式语义漂移**
   ——image+pose 条件钉死场景与轨迹，到 f120 三方仍是同一公园/桥/亭；断崖（f29，
   首次回访点）的本质是量化 run 从 INT2 记忆里**重渲染出"同一场景的平行变体"**
   （石板接缝/树冠/桥栏/倒影结构性错位，天空与平滑区全黑），此后各自锁定永不对齐
   ——崖后 PSNR 测的是平行变体间距离，不是画质。入口：
   `videos/hy_cliff_side_by_side_f10-60_slow4x.mp4`（三方并排慢放）、
   `videos/hy_diffmap_qvg_f10-60_slow4x.mp4`（逐帧差异热图）、
   `figs/diff/`（单帧热图 ×20）。

## 待办五：HY 各向异性死因的靶向拆解（坐标系理论证伪后的新问题）

**背景**：0717 三次修正后（[rope-coordinate-finding.md](rope-coordinate-finding.md)
§四/§五），"读取旋转动态"解释证伪，HY 上 KLT 灾难（16.6）机制开放。

- **实验**：只对 rope 半区做 KLT、prope 半区留均匀格（PCA_KLT_* 复用 + 半区拆分
  已有 PCA_HALF_R 基建）。若仍崩 → 死因在闭环反馈/度量错位；若不崩 → prope
  时变读取度量说得到支持（query 侧 prope 跟当前相机走）。
- **产出**：更新 rope-coordinate-finding.md §四候选清单为裁决结论。
