...
| 9 | **N7：帧级检索概率比特分配**（每量化事件对被量化帧算与全部未来相机的 FOV 重叠 = 未来被检索概率，贪心分配残差比特 b∈[1,4]、均值恒 2，K/V 同施；与管线检索规则同度量、自洽；无 pose 模型退化 N4（逐位验证）） | HY 跑中（temper 1.0 / 2.0 双臂） | — | 直接瞄准断崖机制；INT4 证据（记忆保真→分岔推迟）支持 |
