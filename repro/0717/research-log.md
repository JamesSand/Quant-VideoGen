...
| 12 | **N10：K←SAS 字典（逐事件重拟，fp8 表）+ asym B128 残差；V←N4** | LC 跑中（K=256） | — | 真实现 BPE：LC K侧 2.257 + V侧 2.315 → 均 2.286 ✓；HY（K=128）K侧 2.297 → 均 2.306 ✓。预期 HY K 误差 0.31 vs N4 0.371；K 取"预算内最大"为统一规则 |
