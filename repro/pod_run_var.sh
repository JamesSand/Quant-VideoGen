...
case $M in
  qvgpro)
    torchrun --nproc_per_node=1 --standalone repro/longcat_rngiso_launcher.py $COMMON \
  ...
  quarot_asym16)
    ...
    torchrun --nproc_per_node=1 --standalone repro/quarot_launcher.py $COMMON \
