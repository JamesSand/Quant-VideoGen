...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.local_attn_size == -1:
            ...
            # With raw tensor storage this stays sync-free; with ChunkedKVCache
            # the .write/.read calls do Python-int math but no .item() either.
            ...
            # Sliding-window eviction path. ChunkedKVCache uses read+write to
            # roll content; raw-tensor uses in-place slice copy.
            num_evicted_tokens = num_new_tokens + kv_cache["local_end_index"].item() - kv_cache_size
            num_rolled_tokens = kv_cache["local_end_index"].item() - num_evicted_tokens - sink_tokens
            if hasattr(kv_cache["k"], "read"):
                k_roll = kv_cache["k"].read(
                    sink_tokens + num_evicted_tokens,
                    sink_tokens + num_evicted_tokens + num_rolled_tokens,
                ).clone()
                v_roll = kv_cache["v"].read(
                    sink_tokens + num_evicted_tokens,
                    sink_tokens + num_evicted_tokens + num_rolled_tokens,
                ).clone()
                _kv_write(kv_cache["k"], sink_tokens, sink_tokens + num_rolled_tokens, k_roll)
                _kv_write(kv_cache["v"], sink_tokens, sink_tokens + num_rolled_tokens, v_roll)
            else:
